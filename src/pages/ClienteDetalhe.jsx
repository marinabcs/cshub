  ...
  if (!cliente) {
    return (
      <div className="text-center py-12">
        <p className="text-dark-400">Cliente não encontrado.</p>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h1 className="text-2xl font-bold text-white">{cliente.team_name}</h1>
                  <StatusBadge status={cliente.health_status} />
                </div>
                <p className="text-dark-400">{cliente.team_type}</p>
            ...
            <div className="flex items-center gap-6">
              <div className="text-center">
                ...
                <p className="text-xs text-dark-500 mt-1">Health Score</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-dark-500">Última interação</p>
        ...
        <nav className="flex gap-8">
          {tabs.map((tab) => (
            ...
            >
              {tab.label}
              {tab.count !== undefined && (
                <span className="ml-2 px-2 py-0.5 text-xs bg-dark-700 text-dark-400 rounded-full">
                  {tab.count}
                </span>
              )}
            </button>
          ...
          <CardContent>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm text-dark-400 truncate">{usuario.email}</p>
                {usuario.dominio && (
                  <p className="text-xs text-dark-500">{usuario.dominio}</p>
                )}
              </div>
            </div>
          </CardContent>
      ...
      <Card className="lg:col-span-2">
        ...
        <CardContent>
          ...
          <div className="mt-4 grid grid-cols-4 gap-4 text-center text-sm">
            <div className={`p-3 rounded-xl ${cliente.health_score >= 80 ? 'bg-emerald-500/20 border border-emerald-500/30' : 'bg-dark-700/50'}`}>
              <p className={`font-medium ${cliente.health_score >= 80 ? 'text-emerald-400' : 'text-dark-400'}`}>80-100</p>
              <p className="text-xs text-dark-500">Saudável</p>
            </div>
            <div className={`p-3 rounded-xl ${cliente.health_score >= 60 && cliente.health_score < 80 ? 'bg-amber-500/20 border border-amber-500/30' : 'bg-dark-700/50'}`}>
              <p className={`font-medium ${cliente.health_score >= 60 && cliente.health_score < 80 ? 'text-amber-400' : 'text-dark-400'}`}>60-79</p>
              <p className="text-xs text-dark-500">Atenção</p>
            </div>
            <div className={`p-3 rounded-xl ${cliente.health_score >= 40 && cliente.health_score < 60 ? 'bg-orange-500/20 border border-orange-500/30' : 'bg-dark-700/50'}`}>
              <p className={`font-medium ${cliente.health_score >= 40 && cliente.health_score < 60 ? 'text-orange-400' : 'text-dark-400'}`}>40-59</p>
              <p className="text-xs text-dark-500">Risco</p>
            </div>
            <div className={`p-3 rounded-xl ${cliente.health_score < 40 ? 'bg-red-500/20 border border-red-500/30' : 'bg-dark-700/50'}`}>
              <p className={`font-medium ${cliente.health_score < 40 ? 'text-red-400' : 'text-dark-400'}`}>0-39</p>
              <p className="text-xs text-dark-500">Crítico</p>
            </div>
          </div>
        </CardContent>
      </Card>
